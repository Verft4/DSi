...
class _GeneratorPageState extends State<GeneratorPage> {
  List<List<dynamic>> jogos = [];
  int currentIndex = 0;
  List<int> favoritos = []; // Lista de favoritos
  ...
  Widget build(BuildContext context) {
    ...
    final appid = jogoAtual[0];
    final header = jogoAtual[12];
    final genres = jogoAtual[35];
    final name = jogoAtual[1];
        ...
        if (details.primaryVelocity != null) {
          setState(() {
            if (details.primaryVelocity! > 0) {
              // Arrastar para baixo: adicionar aos favoritos
              ...
              // Arrastar para cima: próximo jogo
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                  ...
                  image: DecorationImage(
                    ...
                    fit: BoxFit.fill, // Ajusta a imagem ao tamanho do container
                  ),
                ),
              ),
            ),
      ...
      children: [
        ...
        Expanded(
          child: searchQuery.isEmpty
              ? Center(child: Text('Digite para buscar um jogo.'))
              : ListView.builder(
                  itemCount: jogosFiltrados.length,
                  itemBuilder: (context, index) {
                    return ListTile(
                      title: Text(jogosFiltrados[index][1]),
                      onTap: () {
                        setState(() {
                          selectedIndex = index;
                        });
        ...
        if (selectedIndex != null)
          Jogos(jogo: jogosFiltrados[selectedIndex!]),
      ],
    );
  }
}
...
class _QuizPageState extends State<QuizPage> {
  ...
  String? plataformaPreferida;

  List<String> tags = [];
  List<String> generos = [];
  List<String> plataformas = ['Windows', 'Linux', 'Mac']; // Adicionadas manualmente
  ...
  Future<void> _salvarRespostaNoFirestore() async {
    // Instância do Firestore
    final firestore = FirebaseFirestore.instance;

    // Salvar as respostas no Firestore
    await firestore.collection('respostas_quiz').add({
      'tagFavorita': tagFavorita,
      'generoFavorito': generoFavorito,
      'preferenciaJogos': preferenciaJogos,
      'plataformaPreferida': plataformaPreferida,
    });
              ...
              child: Padding(
                ...
                child: Form(
                  ...
                  child: PageView(
                    ...
                    children: [
                      // Primeira pergunta: Tag Favorita
                      _buildQuestionCard(
                        'Digite uma tag que você gosta (ou deixe em branco para ignorar)',
                        TextFormField(
                          decoration: InputDecoration(
                            border: OutlineInputBorder(
                              borderRadius: BorderRadius.circular(15),
                      ...
                      // Segunda pergunta: Gênero Favorito
                      _buildQuestionCard(
                        'Digite um gênero que você gosta (ou deixe em branco para ignorar)',
                        Autocomplete<String>(
                          optionsBuilder: (TextEditingValue textEditingValue) {
                            if (textEditingValue.text.isEmpty) {
                              return const Iterable<String>.empty();
                      ...
                      // Terceira pergunta: Preferência de Jogos
                      _buildQuestionCard(
                        'Você prefere jogos Single-Player ou Multi-Player?',
                        DropdownButtonFormField<String>(
                          ...
                          items: ['Single-Player', 'Multi-Player']
                              .map((String value) {
                      ...
                      // Quarta pergunta: Plataforma Preferida
                      _buildQuestionCard(
                        ...
                        DropdownButtonFormField<String>(
                          ...
                          items: plataformas.map((String value) {
                            return DropdownMenuItem<String>(
                              value: value,
                              child: Text(value),
                            );
                          ...
                          onChanged: (value) {
                            setState(() {
                              plataformaPreferida = value;
                            });
                          },
                        ),
                        () {
                          _salvarRespostaNoFirestore(); // Salva as respostas no Firestore
                        },
                      ),
                    ],
                  ),
  ...
  Widget _buildQuestionCard(String question, Widget inputField, VoidCallback onNext) {
    return Card(
      shape: RoundedRectangleBorder(
        borderRadius: BorderRadius.circular(15),
      ),
